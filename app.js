    ...
    const BASE_PATH = (_pathFirst === 'zanhua') ? '/zanhua' : '';
    const API_HOST = 'https://154.201.81.86';
    const API_BASE = API_HOST + BASE_PATH + '/api';
    const DEFAULT_AVATAR = API_HOST + BASE_PATH + '/uploads/default_avatar.webp';
    const PAY_QR_URL = API_HOST + BASE_PATH + '/res/pay/wechat_pay_qr.webp';
    ...
    function resolveMediaUrl(url) {
      ...
      return API_HOST + BASE_PATH + '/' + s.replace(/^\/+/, '');
    }
    ...
    function getVerifSvg(type, size = 14, inline = false) {
      ...
      if (type === 'enterprise') {
        return `<img src="${API_HOST + BASE_PATH}/res/icons/icon-i5xq4thdo.svg" style="${s};filter:invert(35%) sepia(94%) saturate(1587%) hue-rotate(185deg) brightness(97%) contrast(95%);" class="verif-icon verif-enterprise" alt="认证">`;
      }
      if (type === 'basic') {
        return `<img src="${API_HOST + BASE_PATH}/res/icons/icon-i5xq4thdo.svg" style="${s};filter:invert(52%) sepia(28%) saturate(614%) hue-rotate(86deg) brightness(94%) contrast(90%);" class="verif-icon verif-basic" alt="普通认证">`;
      }
      if (type === 'personal') {

        return `<img src="${API_HOST + BASE_PATH}/res/icons/icon-i5xq4thdo.svg" style="${s};filter:invert(68%) sepia(98%) saturate(2000%) hue-rotate(10deg) brightness(95%) contrast(105%);" class="verif-icon verif-personal" alt="认证">`;
      }
      if (type === 'advanced') {
        return `<img src="${API_HOST + BASE_PATH}/res/icons/icon-jztvozsrv.svg" style="${s};filter:grayscale(100%) brightness(0.82) contrast(1.15);" class="verif-icon verif-advanced" alt="进阶认证">`;
      }
      if (type === 'premium') {
        return `<img src="${API_HOST + BASE_PATH}/res/icons/icon-jztvozsrv.svg" style="${s}" class="verif-icon verif-premium" alt="高级认证">`;
      }
      return '';
    }
    ...
    function renderPostCard(p) {
      ...
      return `<div class="${cardClass}" onclick="goPostDetail('${p.id}')">
        <div class="post-header">
          <img class="avatar" src="${p.avatar||DEFAULT_AVATAR}" onclick="event.stopPropagation();goUserProfile('${p.user_id}')" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
          <div class="post-user">
    ...
    function renderConfessionCard(c) {
      ...
      const avatar = showUser ? (c.avatar || DEFAULT_AVATAR) : DEFAULT_AVATAR;
      ...
      try {
        ...
        list.innerHTML = res.data.map(n => {
          ...
          return `<div class="notify-item" style="display:flex;padding:12px 16px;border-bottom:0.5px solid #f0f0f0;">
            <img src="${n.avatar || DEFAULT_AVATAR}" onclick="goUserProfile('${n.from_user}')" style="width:44px;height:44px;border-radius:50%;flex-shrink:0;cursor:pointer;" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
    ...
    function renderConfessionDetail() {
      ...
      return `<div class="post-detail" style="background:#fff;min-height:100vh;">
        ...
        <div class="comment-input-bar">
          <img class="comment-input-avatar" src="${myAvatar || DEFAULT_AVATAR}" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
    ...
    async function loadConfessionComments(confessionId) {
      ...
      const renderComment = (c, repliesHtml, parentName) => {
        ...
        return `<div class="comment-item" data-ccomment-id="${c.id}" data-cis-mine="${isMine}">
            <img class="c-avatar" src="${c.avatar||DEFAULT_AVATAR}" onclick="goUserProfile('${c.user_id}')" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
            <div class="c-body">
              <div class="c-header">
                ${nameHtml}
    ...
    function renderHome() {
      return `<div class="page">
        ...
        <div id="postList"></div>
        <div class="loading" id="loadMore" style="display:none;text-align:center;padding:20px;color:#999;">加载中...</div>
    ...
    function openHomeworkModal() {
      ...
      if (avatarImg) avatarImg.src = myAvatar || DEFAULT_AVATAR;
        ...
        if (res.code === 1) {
          ...
          if (list.length === 0) {
            ...
            listEl.innerHTML = list.map(item => {
              const imgs = item.images ? item.images.split(',').filter(x => x).map(img => img.includes('/') ? img : BASE_PATH + '/uploads/homework/' + img) : [];
              const imgClass = imgs.length === 1 ? 'single' : '';
              let imagesHtml = '';
              if (imgs.length > 0) {
                if (imgs.length <= 9) {
              ...
              return `
                <div class="card" onclick="goHomeworkDetail(${item.id})">
                  <div class="post-header">
                    <img class="avatar" src="${item.avatar ? item.avatar : DEFAULT_AVATAR}" onclick="event.stopPropagation();goUserProfile('${item.user_id}')" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
    ...
    async function bindHomeworkDetailEvents() {
      ...
      const imgs = homeworkDetail.images ? homeworkDetail.images.split(',').filter(x => x).map(img => img.includes('/') ? img : BASE_PATH + '/uploads/homework/' + img) : [];
      const imgClass = imgs.length === 1 ? 'single' : '';
      content.innerHTML = `
        <div style="padding-top:0;">
          <div class="post-header">
            <img class="avatar" src="${homeworkDetail.avatar ? homeworkDetail.avatar : DEFAULT_AVATAR}" onclick="goUserProfile('${homeworkDetail.user_id}')" style="cursor:pointer;" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
    ...
    async function loadHomeworkComments() {
      ...
      const renderHwComment = (c, repliesHtml, parentName) => {
        ...
        const isMine = c.user_id === myUid || currentNickname === '管理员';
        const avatar = c.avatar ? c.avatar : DEFAULT_AVATAR;
      ...
      if(res.code === 1) {
        ...
        list.innerHTML = res.data.map(u => `
          <div class="user-select-item" onclick="toggleUserSelect('${u.uid}')">
            <img src="${u.avatar || DEFAULT_AVATAR}" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
      ...
      if (res.code === 1) {
        ...
        list.innerHTML = res.data.map(u => `
          <div class="user-select-item" onclick="togglePostUserSelect('${u.uid}')">
            <img src="${u.avatar || DEFAULT_AVATAR}" style="width:36px;height:36px;border-radius:50%;" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
            <div class="info">${u.nickname}</div>
            <div class="check ${targetList.includes(u.uid) ? 'checked' : ''}"><i class="${targetList.includes(u.uid) ? 'fa-solid fa-circle-check' : 'fa-regular fa-circle'}"></i></div>
          </div>
    ...
    function renderPostDetail() {
      ...
      let html = `<div class="post-detail" style="background:#fff;min-height:100vh;">
        ...
        <div class="pd-layout" style="padding-top:calc(50px + env(safe-area-inset-top));">
          <div class="pd-main">
          <div class="post-header">
            <img class="avatar" src="${p.avatar||DEFAULT_AVATAR}" onclick="goUserProfile('${p.user_id}')" style="cursor:pointer;" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
    ...
    function renderDiscover() {
      ...
      return `<div class="page">${renderNavbar('发现', false)}
        ...
        <div style="margin:12px 16px 0;background:linear-gradient(135deg,#1C1C1E,#2C2C2E);border-radius:14px;padding:16px;display:flex;align-items:center;justify-content:space-between;cursor:pointer;" onclick="goPage('verifSubscribe')">
          <div style="display:flex;align-items:center;gap:10px;">
            <img src="${API_HOST + BASE_PATH}/res/icons/icon-jztvozsrv.svg" style="width:40px;height:40px;" alt="认证">
            <div>
    ...
    async function loadDiscoverContent(tab) {
      ...
      content.innerHTML = '<div class="loading" style="text-align:center;padding:20px;">加载中...</div>';
      try {
        if (tab === 'hot') {
            ...
            <div class="modal-overlay" id="homeworkModal" onclick="if(event.target===this)closeHomeworkModal()">
              <div class="modal-content" style="max-height:90vh;overflow-y:auto;box-sizing:border-box;">
                ...
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;padding-bottom:12px;border-bottom:0.5px solid #f0f0f0;">
                  <img id="hwModalAvatar" src="${myAvatar || DEFAULT_AVATAR}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
        ...
        if (!chatRes.data || chatRes.data.length === 0) {
          ...
          list.innerHTML = chatRes.data.map(c => {
            ...
            <div class="chat-list-item" onclick="goChat('${c.otherUser}', ${c.is_anonymous ? 'true' : 'false'})" style="display:flex;align-items:center;padding:12px 16px;border-bottom:0.5px solid #f0f0f0;cursor:pointer;background:#ffffff !important;background-color:#ffffff !important;box-sizing:border-box;width:100%;">
              <img src="${c.is_anonymous ? DEFAULT_AVATAR : (c.avatar||DEFAULT_AVATAR)}" style="width:44px;height:44px;border-radius:50%;flex-shrink:0;" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
              <div style="flex:1;margin-left:12px;overflow:hidden;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
            ...
            container.innerHTML = res.data.map(c => {
              ...
              return `<div class="card" onclick="goConfessionDetail(${c.id})" style="margin:0 8px 8px;">
                <div class="post-header" style="padding:10px 12px;">
                  <img class="avatar" src="${c.is_anonymous ? DEFAULT_AVATAR : (c.avatar||DEFAULT_AVATAR)}" style="width:32px;height:32px;" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
                  <div class="post-user">
    ...
    async function bindProfileEvents() {
      try {
        ...
        if (res.code === 1) {
          ...
          myAvatarEl.src = res.data.avatar || DEFAULT_AVATAR;
    ...
    function renderSearchUserCard(u) {
      return `<div class="user-select-item" onclick="goUserProfile('${u.uid}')" style="cursor:pointer;padding:12px 16px;border-bottom:0.5px solid #f0f0f0;">
        <img src="${u.avatar || DEFAULT_AVATAR}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
    ...
    function renderUserProfile() {
      ...
      return `<div class="page">
        ...
        <div class="profile-header">
          <div class="profile-top">
            <img class="profile-avatar" src="${u.avatar||DEFAULT_AVATAR}" style="width:60px;height:60px;border-radius:50%;" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
            <div class="profile-info">
    ...
    async function loadChatMessages() {
      try {
        ...
        if (res.code === 1) {
          ...
          const otherAvatar = chatUserProfile ? (chatUserProfile.avatar || DEFAULT_AVATAR) : DEFAULT_AVATAR;
          const myAvatarUrl = myAvatar || DEFAULT_AVATAR;
    ...
    async function bindEditProfileEvents() {
      ...
      try {
        ...
        if (res.code === 1) {
          ...
          if (avatarEl) avatarEl.src = u.avatar || DEFAULT_AVATAR;
    ...
    async function uploadNewAvatar(file) {
      ...
      try {
        ...
        if (res.code === 1) {
          document.getElementById('editAvatar').src = res.data.avatar;
          showToast('头像上传成功');
        } else {
          if (res.msg && res.msg.indexOf('涉嫌') !== -1) {
            showViolationBubble('已违规');
    ...
    function renderFollowListItems(list, isMine) {
      ...
      content.innerHTML = list.map(u => {
        ...
        return `<div style="display:flex;align-items:center;padding:12px 16px;border-bottom:0.5px solid #f0f0f0;${isBatch ? 'cursor:pointer;' : ''}" ${isBatch ? `onclick="toggleFollowSelect('${u.uid}')"` : ''}>
          ...
          <img src="${u.avatar||DEFAULT_AVATAR}" style="width:44px;height:44px;border-radius:50%;object-fit:cover;flex-shrink:0;" onerror="this.src='${DEFAULT_AVATAR}';this.onerror=null">
          <div style="flex:1;margin-left:12px;overflow:hidden;">
    ...
    window.uploadEntFiles = async function(input) {
      ...
      try {
        const r = await fetch(BASE_PATH + '/api/uploadEnterpriseFile', { method: 'POST', body: fd, headers: { 'Authorization': getToken() } });
